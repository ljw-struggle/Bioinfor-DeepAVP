...
def inference(data, sequence_length, keep_prob):
    ...
    with tf.variable_scope('conv_pssm') as scope:
        matrix = sio.loadmat('./data/all_whsx_list.mat')['all_whsx']
        matrix = np.transpose(matrix)
        initializer_filters = tf.reshape(tf.constant(matrix, dtype=tf.float32), [1, 20, 1, 57])
        ...
        biases = tf.get_variable('biases', [57], initializer=initializer_biases, dtype=tf.float32, trainable=False)
        ...
        conv_pssm = temp_b  # shape= [batch_size, 107, 1, 57]
        ...
        def body(i, array):
            avblock_temp = tf.reshape(conv_pssm[i][0:width[i]], [4, -1, 57])
            avblock = tf.reshape(tf.reduce_mean(avblock_temp, axis=1), [4, 57])
            array = array.write(i, avblock)
            return i + 1, array

        i, array = tf.while_loop(cond, body, (0, Tensor_array))
        outputs = array.stack() # shape = [batch_size, 4, 57]
    ...
    with tf.variable_scope('conv_feature_extraction') as scope:
        ...
        initializer_biases = tf.constant_initializer(0)
        filters = tf.get_variable('filters', [4, 3, 1, 20], initializer=initializer_filters, dtype=tf.float32)
        biases = tf.get_variable('biases', [20], initializer=initializer_biases, dtype=tf.float32)

        input = tf.reshape(outputs, [batch_size_op, 4, 57, 1])
        temp = tf.nn.conv2d(input, filters, strides=[1, 4, 3, 1], padding='SAME')
        temp_b = tf.nn.bias_add(temp, biases)
        conv_feature_extraction = tf.nn.relu(temp_b)  # shape= [batch_size, 1, 19, 20]
    ...
    with tf.variable_scope('Merge_features') as scope:
        conv = tf.reshape(dropout, [batch_size_op, 380])
        merge_features = tf.concat([lstm_outputs, conv], axis=1) # shape = [batch_size, 508]
    ...
    with tf.variable_scope('fully_connected_1') as scope:
        ...
        weights = tf.get_variable('weight', [508, 100], initializer=initializer_weights, dtype=tf.float32)
