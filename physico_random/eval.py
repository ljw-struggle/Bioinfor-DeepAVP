...
tf.flags.DEFINE_integer('validation_num', 1, """...""")
...
def eval():
    with tf.Graph().as_default():
        ...
        with tf.Session(config=session_config) as sess:
            ...
            sio.savemat('physico_random.mat', {'pred': prediction_probability_test, 'label':test_label})

            # print('eval accuracy = %.2f%%' % (acc * 100.0))

            # M = sess.run(fine_tuned_matrix)
            # M = np.reshape(M, [20, 57])
            # sio.savemat('./physico_result_'+ str(FLAGS.validation_num)+ '.mat', {'physico': M})
